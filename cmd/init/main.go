...
func main() {
	...
	defer func() {
		...
		// i think i read somewhere that firecracker does not restart, it just
		// shuts down when you try to reboot
		log.Info("trying to reboot system")
	...
	config, err := DecodeMachine("/spitfire/run.json")
	if err != nil {
